...
def test_no_new_routes_and_docs_present():
    ...
    assert len([r for r in app.routes]) == 1090  # +3 client workspace task/note mutations  # +1 document resolve-ownership  # +1 multi-select access-profile editor  # +9 employee & access management  # +2 staff home dashboard (staff-home-dashboard-mvp)  # +3 work-item detail (work-engine-ui)  # +13 client portal API (client-portal-mvp)  # +8 client vault (client-vault-mvp)  # ... +7 unified work queue (D.39) +1 household 360 (D.41) +7 advisor AI assist (D.42)
