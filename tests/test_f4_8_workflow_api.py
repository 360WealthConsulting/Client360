...
def test_new_routes_registered_and_inventory():
    ...
    assert len(app.routes) == 1207  # +6 canonical document publication (POST /api/publications, POST /api/publications/{id}/revoke, POST /api/publications/{id}/archive, GET /api/documents/{id}/publications, GET /api/publications/preview, GET /api/v1/portal/publications/{id}/download)  # +1 client portal landing page (GET /portal)  # +1 reviewed lead import (POST /lead-import/{message_id})  # +1 Microsoft message-detail preview (/microsoft365/mail/{message_id})  # +1 staff task-list page (/operations/task-list)  # +3 workspace document delete (person/household/business)  # +3 workspace document upload (person/household/business)  # +2 email one document from the staff user's mailbox (GET+POST /documents/{id}/email)  # +2 admin business-ownership resolution (review + apply)  # +8 payroll hub foundation (7 staff + 1 client portal)  # +14 billing & invoicing (12 staff + 2 client portal)  # +4 communication hub staff routes (assign/resolve/link-request/create-request)  # +6 client feature & access control (staff access panel GET+3 POST, admin feature-controls GET+POST)  # +4 client portal browser UI (documents/upload GET+POST, browser logout)  # +1 staff portal-invite HTML form; +1 unified review inbox (readiness)  # +2 admin bulk-confirm HIGH preview+execute (Phase 2)  # +1 admin per-document resolution (resolve-document)  # +1 document image preview (HEIC/HEIF)  # +1 document workbook preview (admin review)  # +1 admin unassigned-documents review (inspect-before-assign)  # +1 admin unassigned-documents folder-resolution (preview/confirm)  # +1 admin unassigned-documents queue (relocated out of client Documents tab)  # +4 Drake identity-review (merge-forward: /matches/drake GET + approve/reject/defer); +3 client workspace task/note mutations  # +1 document resolve-ownership  # +1 multi-select access-profile editor  # +9 employee & access management  # +2 staff home dashboard (staff-home-dashboard-mvp)  # +3 work-item detail (work-engine-ui)  # +13 client portal API (client-portal-mvp)  # +8 client vault (client-vault-mvp)  # ... +7 compliance intelligence / supervisory ops (D.47) +8 executive reporting (D.48)  # +2 external IdP portal auth (start + callback)  # +1 staff client-search for the portal invite form (GET /admin/client-portal/client-search)  # +1 staff Add New Client creation (POST /admin/client-portal/create-client)  # +5 client email one-time-code sign-in (POST /portal/login, GET /portal/activate, GET+POST /portal/verify, POST /portal/verify/resend)  # -2 removed client Microsoft auth (/portal/auth/start, /portal/auth/callback)  # +1 staff-initiated secure message (POST /admin/client-portal/threads/new)  # +1 SharePoint change-notification webhook (POST /api/microsoft/sharepoint/webhook)  # +2 client Documents preview drawer (GET /client/{person_id}/documents/{document_id}/panel, GET /client/household/{household_id}/documents/{document_id}/panel)  # +2 read-only MCP interface (POST + GET /mcp)  # +3 staff notification inbox (GET /notifications, POST /notifications/{id}/read, POST /notifications/read-all)  # +2 staff Outlook reply (GET+POST /communications/messages/{message_id}/reply)  # +1 staff communications inbox (GET /communications/inbox)  # +2 3CX custom-CRM connector (POST /api/integrations/3cx/lookup, POST /api/integrations/3cx/calls)  # +3 continuous document pipeline operations (GET /api/document-pipeline/metrics, /health, /blockers)
