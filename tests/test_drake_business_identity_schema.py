...
def test_the_migration_graph_has_exactly_one_head():
    ...
    # short-row re-key) -> ``docpipe01`` -> ``docpub01`` -> ``docpipe02`` (source-level review) ->
    # ``ocrclaim01`` (parallel-OCR work claims); see ``tests/test_drake_1120s_short_row_repair.py``.
    assert heads == {"ocrclaim01"}, f"expected exactly one head (ocrclaim01), found {sorted(heads)}"
