...
def test_ocr_extraction_stores_text_against_canonical(tmp_path):
    did = _doc(sha="a" * 64)
    s = ocr.run_ocr(isolate=False, document_ids=[did], extractor=_text("Form 1040 line 1 wages"), actor_user_id=_UID)
...
def test_unsupported_type_is_skipped_not_failed():
    ...
    s = ocr.run_ocr(isolate=False, document_ids=[did], extractor=_text("x"))
...
def test_search_matches_ocr_text():
    ...
    ocr.run_ocr(isolate=False, document_ids=[did], extractor=_text(f"balance sheet {token} total"))
...
def test_enrichment_creates_no_new_document_rows():
    ...
    ocr.run_ocr(isolate=False, document_ids=[did], extractor=_text("enriched"))
    with engine.connect() as c:
...
def test_rerun_does_not_duplicate_ocr_rows():
    did = _doc(sha="c" * 64)
    ocr.run_ocr(isolate=False, document_ids=[did], extractor=_text("first"))
    ocr.run_ocr(isolate=False, document_ids=[did], extractor=_text("second"), mode="reprocess")
    with engine.connect() as c:
...
def test_incremental_skips_completed_unchanged():
    did = _doc(sha="d" * 64)
    ocr.run_ocr(isolate=False, document_ids=[did], extractor=_text("done"))
    # An incremental pass must not re-OCR an already-completed, unchanged document.
    s = ocr.run_ocr(isolate=False, document_ids=[did], mode="incremental", extractor=_text("SHOULD NOT OVERWRITE"))
...
def test_reprocess_on_content_change():
    did = _doc(sha="e" * 64)
    ocr.run_ocr(isolate=False, document_ids=[did], extractor=_text("v1"))
    with engine.begin() as c:                                 # canonical content changed
        c.execute(documents.update().where(documents.c.id == did).values(sha256="f" * 64))
    s = ocr.run_ocr(isolate=False, document_ids=[did], mode="reprocess", extractor=_text("v2"))
...
def test_failure_is_recorded_and_retryable():
    ...
    ocr.run_ocr(isolate=False, mode="retry", extractor=_text("recovered"))
    row2 = _ocr_row(did)
...
def test_retry_respects_max_attempts():
    did = _doc(sha="2" * 64)
    for _ in range(3):
        ocr.run_ocr(isolate=False, mode="retry" if _ else "initial", document_ids=[did] if not _ else None,
                    extractor=_boom, max_attempts=3)
    # After 3 failed attempts, a retry sweep no longer selects it.
    s = ocr.run_ocr(isolate=False, mode="retry", extractor=_boom, max_attempts=3)
...
def test_ocr_search_respects_record_scope():
    ...
    ocr.run_ocr(isolate=False, document_ids=[dm, dt], extractor=_text(f"secret {token} data"))
    with engine.begin() as c:
...
def test_documents_tab_surfaces_ocr_status_and_searchable():
    ...
    ocr.run_ocr(isolate=False, document_ids=[did], extractor=_text("indexed content"))
...
def test_run_is_audited():
    ...
    did = _doc(sha="9" * 64)
    ocr.run_ocr(isolate=False, document_ids=[did], extractor=_text("audited"), request_id="ocr-t", actor_user_id=_UID)
    with engine.connect() as c:
...
def test_dry_run_makes_no_changes():
    did = _doc(sha="8" * 64)
    s = ocr.run_ocr(isolate=False, document_ids=[did], extractor=_text("nope"), dry_run=True)
