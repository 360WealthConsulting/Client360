...
def test_person_mismatch_detail_carries_ids_names_and_sources():
    ...
    g = _detail_for(sha)
    assert g and g["primary_reason"] == "ownership_person_mismatch"
    assert g["member_count"] == 2 and g["excess_rows"] == 1
    assert g["proposed_survivor"] == min(a, b)
    ...
    assert by_doc[a]["is_survivor"] is (a < b)
...
def test_organization_mismatch_detail_resolves_organization_names():
    ...
    g = _detail_for(sha)
    assert g["primary_reason"] == "ownership_organization_mismatch"
...
def test_reason_filter_restricts_the_population():
    ...
    only_person = dm.blocked_details(reasons=["ownership_person_mismatch"])
    codes = {g["primary_reason"] for g in only_person["groups"]}
    assert codes == {"ownership_person_mismatch"}
    assert _detail_for(sha_household, only_person) is None
    assert only_person["filter_reasons"] == ["ownership_person_mismatch"]
...
def test_a_giant_shared_content_group_is_summarised_without_dumping_every_member():
    ...
    g = _detail_for(sha)
    assert g["member_count"] == 40 and g["excess_rows"] == 39
...
def test_blocked_summary_reports_size_buckets_and_largest_groups():
    ...
    assert su["by_reason"]["ownership_person_mismatch"]["groups"] >= 2
...
def test_csv_output_has_one_row_per_member_with_group_context(tmp_path):
    ...
    survivor_row = next(r for r in mine if r["document_id"] == str(min(a, b)))
    assert survivor_row["is_survivor"] == "True"
    assert survivor_row["primary_reason"] == "ownership_person_mismatch"
    assert survivor_row["person_name"] and _TAG in survivor_row["person_name"]
    assert survivor_row["group_distinct_owners"] == "2"
...
def test_blocked_details_reuses_a_supplied_report_without_reclassifying():
    ...
    assert detail["summary"]["blocked_groups_in_this_report"] == report["blocked_groups"]
