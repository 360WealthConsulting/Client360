...
| **4** | Compliance/renewal calendar + SLA/notifications via the existing sweep; honest outcomes. | none |
| **5** | Work Management integration: org/engagement/benefit work items through `work_items()`; queues; capacity/bottlenecks; **no second assignment model**. | data-only (queues) |
