...
3. **Replace the temporary external references** in the SOPs with **controlled citations** (`SRC-ADV-2A`
   / `SRC-IAA` + exact section/clause).
...
| 2 | Schwab account opening | `WLTH-SOP-01` | Policy reframed; operational validation pending | **Externally governed** note added | Procedural flow (from Atlas SOP-006) | External-governance note added | Current opening platform; approval authority; MoneyLink/ACAT currency | none | Account-opening confirmation (by reference) | No — `needs_review` |
