...
|---|---|---|---|---|---|---|
| **Insurance Operations Overview** | H · Insurance Operations | Michael Shelton | Draft | `docs/RELEASE_0.10.0_INSURANCE_ARCHITECTURE.md` | Phases 0–9 (built non-regulated surface) | Publish once the release (Phases 0–9) is RC-validated; describe the operational scope only, exclude all regulated (AD-5) features. |
...
| **Insurance Reporting** | H · Insurance Operations | Michael Shelton | Draft | `app/services/insurance_reporting.py` (`operations_dashboard` + pipeline/reviews/commissions/licensing/exception/work-queue/portal-adoption summaries); routes (`app/routes/insurance.py` — `/api/v1/insurance/dashboard`, `/insurance/dashboard`); SOP draft `docs/confluence/INSURANCE_REPORTING_DASHBOARD_SOP_DRAFT.md` | Phase 2 (counts) + **Phase 8 (dashboard, built)** | Firm-internal staff dashboard; **proportional to the viewer's capabilities & record scope**; operational/workflow/financial only, **no compliance metrics** (AD-5); not the client portal. Publish when Phase 8 is RC-validated. |
...
| **Insurance Integrations (Extension Points)** | H · Insurance Operations / L · Technology & Cybersecurity | Michael Shelton | Draft — reference | `app/services/insurance_integrations.py` (disabled ports); `docs/RELEASE_0.10.0_INSURANCE_ARCHITECTURE.md` §12f; SOP draft `docs/confluence/INSURANCE_INTEGRATIONS_REFERENCE_DRAFT.md` | Phase 9 (non-regulated, built as disabled stubs) | **Reference only** — documents the vendor-neutral, **disabled-by-default** extension points. No live integration, credentials, endpoints, or scheduled jobs; nothing to operate yet. Do not describe any feed as available; activation is a future release + AD-5 clearance for regulated data. |
