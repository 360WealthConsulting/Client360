...
| Test coverage | 🟡 | 1217 tests passing (service + route level). No coverage % gate yet; browser E2E is smoke-level. |
...
| Deployment verification | 🟡 | Runbook + gates + **`scripts/smoke.sh`** (post-deploy verification, verified live) + **`scripts/rollback.sh`** (migration downgrade, verified end-to-end) + reversible migrations. Remaining in-repo: build artifact (RC-2) + deploy automation (RC-4). **Outstanding (ops):** staging/prod deploy execution. See `RC_READINESS.md`. |
