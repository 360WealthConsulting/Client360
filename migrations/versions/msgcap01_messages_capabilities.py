...
``/admin/client-portal/diagnostics`` is NOT under ``/threads`` and is deliberately untouched: it keeps
the generic ``^/admin`` -> ``identity.manage`` rule plus its own ``observability.audit`` dependency.
