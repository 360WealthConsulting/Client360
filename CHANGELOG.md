...
### Added
- Centralized server-side OpenAI provider (`app/services/openai_provider.py`) — **Phase 1: the provider only; no consumer uses it and no document is read, renamed or classified by it.** One seam owns the credential, the model, the timeout, the retry budget and the retention flag, so an OpenAI call can never be scattered into a route or a document service. **Off by default** (`OPENAI_ENABLED`, default false): with it off, or with `OPENAI_API_KEY`/`OPENAI_MODEL` unset, every call short-circuits to a `disabled` / `not_configured` result *before a client is built*, so the app starts and behaves exactly as before. Uses the OpenAI **Responses API** with `store=False` pinned on every request (not a caller-reachable parameter) and no `metadata`/`user`/`safety_identifier`/`prompt_cache_key` field, so no client identifier is transmitted. **No fallback model**: an unset or unavailable `OPENAI_MODEL` is refused, never silently substituted. The API key is read at call time in exactly one function, is never returned, rendered or logged, and no SDK exception text is ever surfaced (fixed sentence + exception class only), with a credential scrubber as the backstop. Failures are structured results, never raises (`auth_failed`, `rate_limited`, `model_unavailable`, `timeout`, `transport_error`, `api_error`, `malformed_response`, `sdk_missing`). Adds a harmless connectivity check that transmits two fixed literals and no client data — `python -m app.services.openai_provider --check` — verifying configuration, enablement, authentication and that the *configured* model answers. Declares the future consumers (document filename normalization, document classification, document understanding, Advisor AI, staff assistance) as a reviewable purpose allow-list; an undeclared purpose is refused. Adds `openai==3.6.0` (imported lazily, so the app still imports on a host without it) and `OPENAI_ENABLED` / `OPENAI_MODEL` / `OPENAI_API_KEY` / `OPENAI_TIMEOUT_SECONDS` / `OPENAI_MAX_RETRIES` configuration with startup warnings for a half-configured provider. No schema migration, no new route.
