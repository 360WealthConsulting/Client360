...
### Added
- **A reviewed 56-document display-name batch can be applied atomically from hash-pinned authority.** `scripts/apply_naming_manifest_56.py` defaults to a read-only dry run and accepts an apply only with the exact authorization phrase tied to the reviewed manifest, owner snapshot, and owner-pinned payload hashes. It rechecks the live normalization preview, locks the exact document set, refuses owner or current-name drift and case-insensitive owner/name collisions, updates only `documents.display_name`, and writes all 56 audit events through the same database connection and transaction. Any failed guard, update, audit write, or in-transaction verification rolls back the entire batch; the runner never renames, moves, or edits a physical file. An apply writes a hash-reported rollback snapshot before the single commit and verifies all 56 names after commit.
- **Staff can edit a client profile, and the Overview is readable again.** The Overview showed a client's details and offered no way to correct them: a wrong phone number or a misspelt name needed a console. It now carries an **Edit Profile** action beside Add Note, gated on `client.write` and record scope like every other staff mutation, opening a form for legal name, preferred name, date of birth, primary phone, primary email, mailing address and household. **Identity stays governed**: `people.correct_person_identity` still owns the legal name, and a new `people.correct_person_birth_date` is the one path that may write `birth_date`, so neither can change as a side effect of an ordinary contact edit. **The household is exactly the one that was chosen.** `households.assign_people_to_household` takes a *set* of people and may create or infer the household they share — right for the bulk tools, wrong for a profile edit — so `people.set_person_household` was added: it validates the target id inside the write transaction, assigns that id and no other, creates nothing, reads no relative's row, and treats "No household" as a distinct, audited selection rather than a blank field. All four writes run in **one transaction**, so a rejected date of birth or a household that vanished between rendering and saving rolls the contact fields back with it; `update_person_contact` now hands its `conn` to the timeline and audit writes so a rolled-back edit cannot leave an audit entry claiming it happened. **A shared email warns and does nothing else** — the form names the other active holders and links to them, and saving moves no correspondence, merges no record and changes no document owner. The Overview itself is now five blocks in one order — Client summary, Needs attention, Financial snapshot, Recent documents, Recent activity — where the five separate "something is wrong here" cards are folded into one **Needs attention** list placed second, above the money, because it is the only block that asks anyone to act. Recent documents and Recent activity stop at five with a View all link into the tab that holds the rest, empty blocks disappear instead of rendering large cards reporting nothing, and `format_datetime` now parses ISO-8601 strings so rows handed over by serializing services read "Mar 4, 2026" rather than `2026-03-04T14:20:35-04:00`. Authorization, organization isolation, document visibility, the existing links and the financial calculations are unchanged.
- **A continuous OCR service, not just a faster lane — and a health gate that actually gates.** PR #302 made the corpus sweepable in parallel but shipped a lane drainer, not a service: `ocr_parallel` runs ONE mode and exits, while `worker.py` loops initial -> retry -> classification forever. Swapping one for the other would have stopped classification dead at 6,256 unclassified documents and never touched the 2,587-document retry lane, and the module docstring claimed otherwise. **`app.jobs.ocr_supervisor` is the missing service**: per pass it drains initial OCR with up to four workers, drains retry OCR through the same claim system, runs classification catch-up, publishes heartbeat/state/counters, and repeats — every lane recomputed from the database, so a kill at any instant resumes correctly and no completed document is reprocessed. **Classification is deliberately NOT parallelised**: `run_knowledge_pipeline` has no claim system and nothing establishes that concurrent invocations are safe, so it keeps worker.py's exact sequential 200-document batches; parallel OCR feeds it faster, it drains as it always did. **The health gate was protecting nothing.** It keyed on `CLIENT360_HEALTH_URL`, which is unset in production, and treated unset as "no opinion" — fail open. It now checks BOTH `/health` and `/readiness` on the local application by default, needs no variable to be correct, and fails closed on unreachable, non-200, or a 200 whose body reports anything but healthy — which is exactly what `/readiness` does when migrations are out of sync mid-deploy. Recovery resumes automatically. Worker count is now clamped to the physical core count rather than merely warned about. Ships a scheduled-task installer that runs S4U so the service survives logout and reboot (the existing task is Interactive and does not), refuses overlapping instances, restarts after an unexpected exit, and exports a restorable copy of the current task before changing anything. Not deployed; the single worker remains the production path.
- **Ownership can be rolled out one source at a time (`DOCUMENT_PIPELINE_OWNERSHIP_SOURCES`).** The phases are not about code readiness, they are about which sources identify a client by a stable key. Drake and TaxDome do — a client id, an account — so a mapping recorded against that key survives a rename. SharePoint identifies a client by evidence inside the document, and evidence is worth more once the authoritative mappings exist, because a folder already resolved to a client stops being a guess. Phase 1 is `DOCUMENT_PIPELINE_OWNERSHIP_SOURCES=drake,taxdome`. An excluded lane is excluded completely: no owner assigned, no review opened, no mapping persisted, checked before any side effect rather than only before the write, because a lane that is "off" but still files rows in a reviewer's queue is not off. Unset means every lane, so adding the switch changes nothing until a host asks it to, and an unrecognised name is dropped with a warning rather than silently widening the scope.

- **A person-owned folder maps to their household only when Drake says they file jointly (`document_pipeline_continuous.household_policy`).** A TaxDome folder owned partly by a person and partly by a household is usually one client recorded at two levels, and occasionally two clients sharing a folder. Nothing structural separates those, so Phase 1 stops guessing and asks an independent authority: a Drake married-filing-jointly return naming both household members. Six conditions must all hold — exactly one active person owner, that person in exactly one active household, Drake MFJ confirmation with the other member, owners limited to that couple and their household, no organization or unrelated owner, and a household whose relationship table and denormalised column agree about its membership. Any failure opens one aggregated folder-level review naming the condition that failed. Drake's `FS` code 2 qualifies and code 3 does not: both carry a spouse on every row, but 3 is married filing *separately*, which is precisely a couple who are not one filing unit.
- **A document with no provenance is held for repair rather than treated as SharePoint evidence.** `detect_lane` answers SharePoint when it finds no source row, because SharePoint is its fallback — but that is the absence of an answer, not an answer. On production, 891 live documents carry a TaxDome folder tag and no `document_sources` row at all, and letting content inference decide those would hand an authoritative-source document to a guess. They now return `provenance_repair_required`, naming the apparent folder, and take no owner, mapping or review.

### Fixed
- **`run_parallel` no longer abandons live OCR workers, so the supervisor can never start a lane beside a still-running generation.** It capped *total generation runtime* at `child_result_timeout=900`. That is the wrong measure: a worker reports exactly once, when its lane ends, so elapsed time says nothing about whether it is healthy — and every corpus lane on a real backlog runs longer than fifteen minutes, so the cap fired **every time**. `break` abandoned live children, `join(60s)` timed out without being fatal, and the synthesis labelled them `startup_failed` with `exitcode None`. On 2026-09-13 the initial lane started 19:39:31; the deadline fired at 19:54:31, four 60-second joins ran to 19:58:31, and the supervisor started the **retry** lane there — beside four workers still claiming and completing documents. The result was **eight concurrent workers against a configured four**, `supervisor_lanes.json` reporting "no OCR work was performed" about workers that had done ~800 documents each, and a generation that would have doubled again at the next lane. Liveness is now measured **per worker from its own last batch ping** (`_worker_entry` installs an `on_batch` hook inside the child, so nothing has to survive pickling), with `DEFAULT_WORKER_STALL_TIMEOUT = 1800s` — safe because `ocr_isolation.run_document` already enforces a per-document hard cap and stall cap and kills the process tree, so no single document can keep a healthy worker silent that long. The return path is now **unconditional on generation liveness**: terminate → escalate to `kill()` → join, and if anything still survives `run_parallel` **raises rather than returns**, because returning is exactly what let the supervisor advance. A stalled worker tears down the **whole** generation first. Non-reporting workers are now classified by what actually happened — `startup_failed` (never alive, non-zero exit), `abnormal_exit`, `exited_without_result`, `runtime_timeout` — and a live worker is never again labelled `startup_failed` with `exitcode None`. The bounded empty-claim retry from PR #309 is preserved, and claim SQL, candidate eligibility, OCR extraction, attempt accounting, schema, migrations, scheduler registration and production configuration are untouched. `supervisor_lanes.json` gains per-outcome counts and the stalled-worker list.
- **A parallel OCR lane no longer retires itself because another worker won the rows it selected.** `ocr_claims.claim_batch` selects `ORDER BY d.id LIMIT <batch>` and wins each row individually, so under contention a worker can win **zero** rows while the lane still holds thousands of documents — the function's own docstring says exactly that, and says to call again. `worker_loop` instead treated the first empty batch as `lane_empty` and returned for good, and nothing replaces a retired worker. In a four-worker run on 2026-09-13 this ended lane `ocr1` after eight minutes with **13,589 documents still outstanding**; its signature was a *clean* exit, not a crash — `done=100, claimed=0`, stopping on an exact batch boundary, no Windows error event, the other three lanes unaffected — so the sweep simply finished three-handed. `worker_loop` now takes **`empty_batch_retries`**, counting *consecutive* empty batches and resetting to zero the moment a batch is won, so only an unbroken run of misses can end a lane. It **defaults to 0 — the historical behaviour — so every existing caller is unchanged**; `app.jobs.ocr_supervisor` opts corpus lanes in at `LANE_EMPTY_RETRIES = 3` (four consecutive empties, `DEFAULT_EMPTY_SLEEP = 2.0s` apart). The retry is deliberately **bounded**: `run_parallel` joins its workers, so a lane that polled forever would never return and the supervisor would never recount and advance to the retry and classification lanes — at genuine exhaustion a worker now costs about six extra seconds against passes measured in hours. Claim SQL, candidate eligibility, OCR behaviour, attempt accounting and the database schema are untouched, and no worker restart or supervision is introduced. **Operator diagnostics**: the supervisor now logs each worker's `stopped_because` and the child exit codes after every `run_parallel`, and persists them to `supervisor_lanes.json` in the existing ops directory through the same atomic publisher — previously `run_parallel` returned both and the supervisor discarded them, so the string `"lane_empty"` existed in memory and was thrown away, and the cause had to be reconstructed from the claims table and process tree hours later. The scheduled-task executable is unchanged; no shell wrapper or stdout redirection was added.
- **The OCR supervisor now holds the in-app sweep's lock, so the scheduled `ocr-incremental-sweep` cannot OCR documents four parallel workers are mid-claim on.** `ocr_parallel` deliberately skipped `ocr_runner`'s per-batch lock `511005002`, reasoning that "per-document claiming is a strictly stronger guarantee". That holds only among claim **participants**. The application's own `ocr-incremental-sweep` job is not one: it runs `ocr_runner.run_sweep` → `document_ocr.run_ocr` and never reads or writes `ocr_document_claims`, while its candidate predicate (`document_ocr.document_id IS NULL OR status IN ('pending','processing')`) is the *same population* the supervisor's initial lane sweeps. Against a non-participant, a claim excludes nothing. The legacy `worker.py` was safe only by accident — it performed OCR *through* `run_sweep`, so it took `511005002` per chunk and the two serialised; it even handles losing that race. A parallel runner calling `run_ocr` directly takes that lock never, so cutting over would have let the scheduled sweep double-OCR claimed documents every 30 minutes, double-bumping `attempts` and spawning extraction subprocesses outside `ocr_throttle`'s admission control — and **no duplicate-claim check could have detected it**, because the sweep takes no claim to duplicate. `app.jobs.ocr_supervisor` now holds **both** `511005888` (single supervisor) and `511005002` (sweep exclusion) for its entire lifetime, on one session, acquired **all-or-nothing before any worker is spawned**, in the fixed order identity-then-resource and released in reverse. If either is unavailable it starts **zero** workers and exits non-zero — `already_running` or `sweep_lock_unavailable` — and the scheduled task's restart policy retries. `511005777` legacy-worker exclusion is unchanged, and PostgreSQL freeing session locks on disconnect remains the crash-safe fallback. No external lock-holder process, no configuration change, and no change to scheduler registration, candidate selection, claims semantics, or OCR behaviour.
- **The OCR supervisor's S4U preflight now validates the source roots Client360 actually needs, instead of a hard-coded list of drive letters.** `install_ocr_supervisor_task.ps1` gated registration on `@('C','D','T')`. Nothing in the repository derives that list — no config default, env var, document or test mentions `T:` — so the check failed hosts that were perfectly able to run the supervisor, and, because `Win32_LogicalDisk` enumerates drive letters only, passed hosts whose required UNC share was unreachable. OCR does not read a drive list: `document_ocr._local_path` opens `storage_uri` then `storage_path` off the row. **`app.deploy.ocr_source_roots` is the authority**, and the required set is the union of two independently-derived halves — the permanent roots the runtime has *effectively* configured (resolved through `app.services.storage_paths`), and the distinct roots that *eligible* document rows actually reference. Neither half alone is safe: the config half misses a drive configuration no longer mentions but documents still live on, and the database half would let a freshly configured root pass merely because nothing has been written there yet. Two exclusions keep it from failing falsely: a permanent root still sitting on its **legacy source-code default** is not required (a literal in a module is not an operator's decision), and an **ingestion-only** root — `TAXDOME_DRIVE_ROOT` (`Z:\`), `DRAKE_EXPORT_ROOT`, the SharePoint staging tree — is not required either, because importers read those and OCR only ever opens the canonical copy. Both are *reported* rather than dropped, and either becomes required the moment eligible rows depend on it. The classification also closes the gaps `DriveType -eq 3` could never see: a **cloud-sync mount** (OneDrive placeholders sit on a local disk but are materialised by a per-user session filter) and a **session mapping** that reports a benign `DriveType` are both rejected, and a required **UNC** root fails closed unless the operator attests the machine account has been granted it — an attestation that answers the credential question only, leaving existence and readability still checked. A failed database read fails the preflight rather than degrading into "nothing required". Separately, `Register-ScheduledTask` piped its result to `Out-Null` and then printed `Registered '<task>'` unconditionally, so an operator without the rights to register a task was told it existed; it now uses `-ErrorAction Stop`, keeps the returned object, re-queries the scheduler, and terminates before any success line.
- **A client whose documents were already filed correctly now gets their mapping recorded, instead of being re-derived forever.** Mappings were learned only as a side effect of writing ownership, so a source identity that needed no write — every document under it already filed to the right client, years ago — stayed unmapped, and the next document arriving under it went back through filename and OCR inference. The best-established clients were getting the least benefit. The already-owned path now reads the agreement its own documents already contain and writes it down once. It assigns nothing, creates nothing and never supersedes an existing mapping. A person and their own household are the same client recorded at two levels and resolve to the household, the rule the TaxDome folder resolver already follows; a person from a *different* household is two clients in one folder, which stays a question rather than becoming a mapping. On production this establishes 1,120 of 1,183 Drake and TaxDome identities.
- **An unresolved TaxDome folder is one question, not one question per file in it.** Ownership conflicts already aggregated onto a single review per source identity; a folder that resolved to no client did not, and still opened a review row per document. On the production corpus that is 3,259 documents across 51 folders — 3,259 items in a queue representing 51 decisions, each restated about 64 times. A queue like that is dismissed rather than read, and the 51 real questions go unanswered. The unresolved-folder path now opens at most one review per stable TaxDome account or folder identity and attaches every affected document to it, reusing the partial unique index and key normalisation the conflict path already uses. No schema change was needed. Everything around it is untouched: a folder that maps to one client still links directly and asks nothing, an already-owned document keeps its owner, Drake and SharePoint still raise one review per document because evidence is about a document's content and two files can be ambiguous for different reasons, and no client record is ever created.
- **The organization profile reaches every document the organization owns, instead of the first 200.** A read-only production trace found 16 TaxDome documents on two organizations that appeared on no staff surface at all: correctly owned, active, unarchived, and sitting past the fixed `limit(200)` in `business_workspace`. Organization-owned documents have no second route — `client_documents` unions person and household anchors only, so a document carrying `organization_id` and nothing else is reachable from the organization profile or from nowhere. The screen made it worse by being quietly self-contradictory: the heading has always rendered the TRUE total, so a business with 514 documents displayed "Documents (514)" above 200 rows, with no pager, no range line, and nothing saying the list was partial. The count was right and the list was wrong, and only the list was actionable. **The cap is replaced by server-side pagination, not by a bigger cap** — a larger fixed number would have moved the cliff rather than removed it, and the largest affected organization already holds 514. Pages are 100 documents; every document is reachable by paging; the heading's total and the reachable set are now the same set. **Ordering became a total order in the process.** `created_at DESC` alone is not one, and bulk Drake/TaxDome imports stamp many rows in the same second; under LIMIT/OFFSET an unstable sort can show one row on two pages and another on none, which is a worse failure than truncation because it is invisible. `id DESC` breaks every tie, so the sequence is stable across pages and across repeated reads. **Related households stopped depending on the page.** They were derived from whichever rows the capped read happened to return, so under paging they would have flickered as a reader moved between pages; they are now computed from all of the organization's documents in one distinct read. Authorization, organization isolation and the active/unarchived lifecycle filter are untouched and are applied to both the page and the count, so this widens reachability without widening visibility — an out-of-range page clamps to a real one rather than 404ing or rendering an empty screen that reads as data loss. No new route, no migration, no schema change. 22 new tests, covering more than five hundred documents on one organization, page navigation and boundaries, total correctness, stable ordering with no duplicates or omissions, archived and deleted exclusion, cross-organization isolation, and Drake and TaxDome treated identically.

### Added
- **Parallel OCR corpus runner — per-document claiming replaces the lock that made the corpus single-threaded.** The full-corpus worker sweeps behind one session-level advisory lock (`511005777`) held for its entire life, so a second copy exits immediately. That lock is the only thing preventing two workers from OCR-ing the same document, and the price of it is that one process does the whole corpus: measured on 360SRV, 4 physical cores at 15.6–23.5% CPU with a disk queue of 0.0 while 31,457 documents remained unattempted. The constraint was never the hardware. **A claim is now taken per document, by one statement** (migration `ocrclaim01`): `ocr_document_claims` holds one row per document whose primary key *is* the mutual-exclusion primitive, and an `INSERT … ON CONFLICT DO UPDATE … RETURNING` gated on `lease_expires_at < now()` means two workers racing for the same document serialise on that index — the loser updates nothing and never receives the document. **Every claim carries a lease, which is what makes crash recovery free**: a worker that is killed, wedged or loses power writes nothing, its lease simply lapses, and the next claimer takes the document over, so there is no cleanup job to run and no state to repair. `claim_seq` increments on every re-claim, so a worker slow enough to lose its lease is detected before it writes and drops its result instead of overwriting the new holder's. A lease covers exactly one document, so a 300-page scan blocks one worker rather than the corpus. **None of the OCR semantics move**: the runner schedules `document_ocr.run_ocr`, the same call the single worker makes, so existing results, the database-authoritative checkpoint, unsupported/encrypted/timeout handling, the classification and retry lanes, duplicate-hash reuse, audit granularity and BELOW_NORMAL priority are preserved by not being touched. `run_sweep`'s per-batch lock is deliberately unused — per-document claiming is strictly stronger, and keeping it would serialise the workers. Admission control (`ocr_throttle.may_claim`) is consulted before each claim and never mid-document, pausing new work on a failing Client360 health endpoint, a memory floor or a CPU ceiling, so throttling can never truncate OCR. `run_parallel` refuses to start while the legacy lock is held. Additive and reversible: the claim table is scheduling metadata, holds no OCR result, and the single worker never reads it, so rollback is stopping the runner and re-enabling the scheduled task. Not deployed; the single worker remains the production path. See `docs/OCR_PARALLEL_RUNNER.md`.
