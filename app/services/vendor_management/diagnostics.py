"""Vendor Management internal diagnostics (Phase D.56) — INTERNAL-ONLY observability of the vendor-management
layer. Composes the gate snapshot, in-process counters, registry coverage, panel availability, and the
governance report into one low-cardinality report for the ``observability.audit`` surface. Exposes NO
contract contents, credentials, license keys, secrets, or procurement payloads — aggregates only.
"""
from __future__ import annotations

from . import gate, registry, stats
from .governance import validate_vendor_management
...
def vendor_diagnostics() -> dict:
    gov = validate_vendor_management()
