"""Financial Operations internal diagnostics (Phase D.57) — INTERNAL-ONLY observability of the
financial-operations layer. Composes the gate snapshot, in-process counters, registry coverage, panel
availability, and the governance report into one low-cardinality report for the ``observability.audit``
surface. Exposes NO payroll details, tax returns, bank account numbers, payment credentials, or accounting
payloads — aggregates only.
"""
from __future__ import annotations

from . import gate, registry, stats
from .governance import validate_financial_operations
...
def financial_diagnostics() -> dict:
    gov = validate_financial_operations()
