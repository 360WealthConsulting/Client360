"""Enterprise Operational Resilience internal diagnostics (Phase D.60) — INTERNAL-ONLY observability of the
resilience layer. Composes the gate snapshot, in-process counters, registry coverage (incl. configured /
not_configured counts + dependency coverage), panel availability, and the governance report into one
low-cardinality report for the ``observability.audit`` surface. Exposes NO sensitive operational payloads —
aggregate metadata only.
"""
from __future__ import annotations

from . import gate, registry, stats
from .governance import validate_operational_resilience
...
def resilience_diagnostics() -> dict:
    gov = validate_operational_resilience()
    ...
    return {
        ...
        "dependency_coverage": cov["operational_dependencies"],
