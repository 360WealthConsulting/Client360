"""Automation Orchestration read-models (Phase D.51).

Normalized, read-only projections of a composed automation panel and an automation dashboard. Every panel
REFERENCES its authoritative owner (owning service + read + deep link) and never mutates, never executes an
automation, never launches a workflow, and never routes an event — the value is composed on READ by the
authoritative owner (the Workflow Engine, the Automation scheduled-job engine, the Trigger engine, the Event
outbox, Scheduling, Communications). Every panel is explainable (explanation + source + deep link); a
non-explainable panel is never emitted. A panel the principal is not entitled to see is emitted
``restricted`` (never its value). **No workflow payloads or client-sensitive automation data are ever carried
in a panel — counts and status only.**
"""
...
class PanelResult:
    ...
    deep_link: str | None      # the authoritative workflow/automation surface to drill into
