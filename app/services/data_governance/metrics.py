"""Data Governance analytics readers (Phase D.52).

These are LOW-CARDINALITY operational counters ABOUT the data-governance layer itself (how many
dashboards/panels/summaries were composed, failures) — registered into the SINGLE, existing Analytics
Registry. This layer defines NO business metrics and NO second metrics registry; every governance count comes
from the authoritative Governance package. Never client-sensitive data, entity payloads, or identifiers.
"""
from __future__ import annotations

from . import registry, stats


def data_governance_metrics(principal=None) -> dict:
    s = stats.governance_stats()
    return {
        "dashboards_composed": s.get("dashboards_composed", 0),
        "panels_composed": s.get("panels_composed", 0),
        "summaries_composed": s.get("summaries_composed", 0),
        "aggregation_failures": s.get("aggregation_failures", 0),
        "authorization_failures": s.get("authorization_failures", 0),
        "restricted_panels": s.get("restricted_panels", 0),
        "avg_compose_ms": s.get("avg_compose_ms", 0.0),
        "by_dashboard": s.get("by_dashboard", {}),
        "governed_entities": registry.coverage()["governed_entities"],
        "stewardship_roles": registry.coverage()["stewardship_roles"],
        "dashboards": registry.coverage()["dashboards"],
        "panels": registry.coverage()["panels"],
    }


# --- readers for the platform Analytics registry (in-process counters; no DB, no PII) ---

def governance_dashboards_composed(principal) -> int:
    return int(stats.governance_stats().get("dashboards_composed", 0))


def governance_panels_composed(principal) -> int:
    return int(stats.governance_stats().get("panels_composed", 0))


def governance_panel_failures(principal) -> int:
    return int(stats.governance_stats().get("aggregation_failures", 0))


def governance_authorization_failures(principal) -> int:
    return int(stats.governance_stats().get("authorization_failures", 0))
