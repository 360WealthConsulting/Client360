"""Data Governance read-models (Phase D.52).

Normalized, read-only projections of a composed data-governance panel and a governance dashboard. Every panel
REFERENCES its authoritative owner (owning service + read + deep link) and never mutates, never merges an
entity, never alters an identity, and never modifies metadata — the value is composed on READ by the
authoritative owner (the D.23 Governance package, the Person-merge / matching engine, the data catalog, the
quality engine). Every panel is explainable (explanation + source + deep link); a non-explainable panel is
never emitted. A panel the principal is not entitled to see is emitted ``restricted`` (never its value).
**No client-sensitive data or entity payloads are ever carried in a panel — counts and status only.**
"""
...
class PanelResult:
    ...
    measure: str               # master_data | stewardship | lineage | ownership | duplicate | validation | quality
    ...
    deep_link: str | None      # the authoritative entity-owner surface to drill into
