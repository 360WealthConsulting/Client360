"""Enterprise Identity & Access Governance internal diagnostics (Phase D.65) — INTERNAL-ONLY observability of
the identity / role / capability / authentication / authorization layer. Composes the gate snapshot, in-process
counters, registry coverage (incl. configured / not_configured counts), panel availability, and the governance
report into one low-cardinality report for the ``observability.audit`` surface. Exposes NO passwords, secrets,
tokens, session IDs, credentials, authentication payloads, raw identities, privileged-role membership, or
user-level permission maps — aggregate metadata only.
"""
from __future__ import annotations

from . import gate, registry, stats
from .governance import validate_identity_governance
...
def identity_diagnostics() -> dict:
    gov = validate_identity_governance()
