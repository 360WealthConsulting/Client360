"""Enterprise Risk Management read-models (Phase D.58).

Normalized, read-only projections of a composed risk/control/assurance panel and a risk dashboard. Every panel
REFERENCES its authoritative owner (owning service + read + deep link) and never creates a risk, changes a
risk rating, closes a finding, approves a control, accepts an exception, acknowledges an incident, assigns
remediation, alters evidence, certifies compliance, or modifies policy — the value is composed on READ by the
authoritative owner (Compliance Intelligence, the Exception Engine, Security Operations / incidents, Data
Governance, the Integration Platform, Business Continuity, Vendor Management, Financial Operations, Document
Intelligence, Automation Orchestration, the Runtime + Policy engines, and audit logging). Every panel is
...
owner-not-configured panel is emitted ``available=False`` (fail closed). **No client-sensitive evidence, audit
payloads, security details, credentials, tokens, bank information, tax-return contents, document contents, or
private incident narratives are ever carried in a panel — counts, status, severity distributions, and
coverage summaries only.** A derived value carries ``derived=True`` and never claims regulatory
certification.
"""
...
class PanelResult:
    ...
    measure: str               # risk | controls | assurance | findings | exceptions | incidents | coverage | remediation
    unit: str                  # count | percent | ratio | status | mixed | distribution
    ...
    deep_link: str | None      # the authoritative risk-owner surface to drill into
    ...
    derived: bool = False      # True when the value is a deterministic derivation (never a certified rating)
