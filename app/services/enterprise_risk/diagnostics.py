"""Enterprise Risk Management internal diagnostics (Phase D.58) — INTERNAL-ONLY observability of the
risk-management layer. Composes the gate snapshot, in-process counters, registry coverage (incl.
configured vs not_configured counts), panel availability, and the governance report into one low-cardinality
report for the ``observability.audit`` surface. Exposes NO client-sensitive evidence, audit payloads, security
details, credentials, tokens, bank information, tax-return contents, document contents, or private incident
narratives — aggregate metadata only.
"""
from __future__ import annotations

from . import gate, registry, stats
from .governance import validate_enterprise_risk
...
def risk_diagnostics() -> dict:
    gov = validate_enterprise_risk()
    s = stats.risk_stats()
    ...
    return {
        ...
        "configured_risk_domains": cov["configured_risk_domains"],
        "not_configured_risk_domains": cov["not_configured_risk_domains"],
