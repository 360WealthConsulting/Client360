"""Business Continuity read-models (Phase D.55).

Normalized, read-only projections of a composed resilience panel and a continuity dashboard. Every panel
REFERENCES its authoritative owner (owning service + read + deep link) and never starts a backup, restores
data, acknowledges an incident, changes monitoring, alters runtime, or modifies infrastructure — the value
is composed on READ by the authoritative owner (the Observability domain, the Runtime engine, the Automation
scheduler, Communications). Every panel is explainable (explanation + source + deep link); a non-explainable
panel is never emitted. A panel the principal is not entitled to see is emitted ``restricted`` (never its
value); a panel whose owner is not configured is emitted ``available=False`` (fail closed).
**No infrastructure payloads or client-sensitive data are ever carried in a panel — counts and status only.**
"""
...
class PanelResult:
    ...
    measure: str               # backup | recovery | restore | infrastructure | runtime | maintenance | notifications | readiness
    ...
    deep_link: str | None      # the authoritative resilience-owner surface to drill into
