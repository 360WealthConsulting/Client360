"""Business Continuity internal diagnostics (Phase D.55) — INTERNAL-ONLY observability of the
business-continuity layer. Composes the gate snapshot, in-process counters, registry coverage, panel
availability, and the governance report into one low-cardinality report for the ``observability.audit``
surface. Exposes NO infrastructure payloads, secrets, or client-sensitive data — aggregates only.
"""
from __future__ import annotations

from . import gate, registry, stats
from .governance import validate_business_continuity
...
def continuity_diagnostics() -> dict:
    gov = validate_business_continuity()
