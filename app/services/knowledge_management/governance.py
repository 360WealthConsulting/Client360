"""Enterprise Knowledge Management governance (Phase D.62) — read-only validation that the knowledge layer
stays a COMPOSITION over the authoritative knowledge / SOP / documentation owners, and never becomes a second
wiki, document-management platform, Confluence replacement, SharePoint, records-management platform, search
engine, AI knowledge store, or document repository. Returns ``{ok, issue_count, findings}`` and NEVER raises
into normal use.

Invariants enforced:
  * No module defines a table / persistence, writes the DB, publishes to the outbox, or writes audit events
    — it only composes reads (no shadow document / SOP / version / knowledge store).
  * No mutation — the layer never calls a document / version / retention mutation (`create_document`,
    `update_document`, `set_status`, `approve`, `archive`, `create_version`, `approve_version`,
    `create_retention_assignment`, `execute_deletion`, `write_audit`).
  ...
  * not_configured entries (SOP governance, runbooks, playbooks, onboarding SOPs, knowledge base, institutional
    memory, wiki, Confluence, search index) are reported honestly; no fabricated document / SOP / version.
...
_AUTHORITATIVE_READS = ("document_platform", "document_intelligence", "governance.retention",
                        "compliance_rule_catalog")

# Mutating/execution entry points this layer must NEVER call (would duplicate a wiki/DMS/version engine).
_FORBIDDEN_CALLS = (
    "create_document(", "update_document(", "set_status(", "approve(", "archive(", "soft_delete(",
    "restore(", "create_folder(", "create_version(", "approve_version(", "restore_version(",
    "create_retention_assignment(", "place_legal_hold(", "execute_deletion(", "link_entity(",
    "unlink_entity(", "write_audit(", "publish_safe(", "publish_event(",
)
...
def validate_knowledge_management() -> dict:
    findings = []
    try:
        for mod in _MODULES:
            s = _src(mod)
        ...
        if "document_platform" not in composed and "document_intelligence" not in composed:
            findings.append({"type": "not_reusing_document_owner"})
        ...
        registries = (
            ("knowledge_domain", registry.KNOWLEDGE_DOMAIN_REGISTRY),
            ("sop_category", registry.SOP_CATEGORY_REGISTRY),
            ("documentation_owner", registry.DOCUMENTATION_OWNER_REGISTRY),
            ("knowledge_source", registry.KNOWLEDGE_SOURCE_REGISTRY),
            ("publication_status", registry.PUBLICATION_STATUS_REGISTRY),
        )
        ...
        for p in registry.PANEL_REGISTRY:
            ...
            # a status / health / coverage panel derived from the layer must be labeled derived.
            layer_derived = p.source.startswith("knowledge_management.")
            if layer_derived and ("status" in p.key or "health" in p.key or "coverage" in p.key
        ...
        dk = [d.key for d in registry.KNOWLEDGE_DASHBOARDS]
