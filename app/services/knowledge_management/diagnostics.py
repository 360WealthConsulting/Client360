"""Enterprise Knowledge Management internal diagnostics (Phase D.62) — INTERNAL-ONLY observability of the
knowledge layer. Composes the gate snapshot, in-process counters, registry coverage (incl. configured /
not_configured counts + documentation / ownership coverage), panel availability, and the governance report into
one low-cardinality report for the ``observability.audit`` surface. Exposes NO document contents, confidential
procedures, credentials, tokens, or client-sensitive documentation — aggregate metadata only.
"""
from __future__ import annotations

from . import gate, registry, stats
from .governance import validate_knowledge_management
...
def knowledge_diagnostics() -> dict:
    gov = validate_knowledge_management()
    ...
    return {
        ...
        "documentation_coverage": cov["knowledge_domains"],
        "ownership_coverage": cov["documentation_owners"],
