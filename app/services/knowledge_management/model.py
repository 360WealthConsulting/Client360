"""Enterprise Knowledge Management read-models (Phase D.62).

Normalized, read-only projections of a composed knowledge / SOP / documentation panel and a knowledge
dashboard. Every panel REFERENCES its authoritative owner (owning service + read + deep link) and never
creates a document, edits a document, approves a document, publishes documentation, changes a version, or
alters metadata — the value is composed on READ by the authoritative owner (the Document Platform document /
version / lifecycle owner, Document Intelligence, Data Governance retention, and Compliance Intelligence).
Every panel is explainable (explanation + source + deep link); a non-explainable panel is never emitted. A
panel the principal is not entitled to see is emitted ``restricted`` (never its value, count, or leaking
metadata); an owner-not-configured panel is emitted ``available=False`` with ``config_status='not_configured'``
(fail closed). **No document contents, confidential procedures, credentials, tokens, or client-sensitive
documentation are ever carried in a panel — counts, status, and coverage summaries only.** A derived value
carries ``derived=True``, describes a documentation-coverage summary (never a fabricated SOP, approval, or
version history), and never invents documentation or institutional knowledge.
"""
...
class PanelResult:
    ...
    measure: str               # knowledge | sop | documentation | ownership | version | publication | freshness | coverage
    ...
    derived: bool = False      # True when the value is a deterministic derivation (summary, never fabricated knowledge)
