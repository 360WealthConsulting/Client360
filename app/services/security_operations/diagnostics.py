"""Security Operations internal diagnostics (Phase D.54) — INTERNAL-ONLY observability of the
security-operations layer. Composes the gate snapshot, in-process counters, registry coverage, panel
availability, and the governance report into one low-cardinality report for the ``observability.audit``
surface. Exposes NO passwords, secrets, tokens, session IDs, or authentication payloads — aggregates only.
"""
from __future__ import annotations

from . import gate, registry, stats
from .governance import validate_security_operations
...
def security_diagnostics() -> dict:
    gov = validate_security_operations()
