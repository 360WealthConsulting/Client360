"""Security Operations read-models (Phase D.54).

Normalized, read-only projections of a composed security panel and a security dashboard. Every panel
REFERENCES its authoritative owner (owning service + read + deep link) and never authenticates, creates a
user, revokes a session, issues a token, resets a password, or alters a permission — the value is composed on
READ by the authoritative owner (the Security metadata domain, Identity, the RBAC foundation, the hash-chain
audit log). Every panel is explainable (explanation + source + deep link); a non-explainable panel is never
emitted. A panel the principal is not entitled to see is emitted ``restricted`` (never its value).
**No passwords, secrets, tokens, session IDs, or authentication payloads are ever carried in a panel —
...
class PanelResult:
    ...
    measure: str               # authentication | authorization | identity | mfa | sessions | audit | posture
    ...
    deep_link: str | None      # the authoritative security-owner surface to drill into
