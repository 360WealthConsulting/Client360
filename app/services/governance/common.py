"""Shared helpers for the Data Governance domain (Phase D.23) — scope, audit, timeline.

Governance owns governance metadata gated by ``governance.*``. Findings/cases/holds/deletion
requests may carry an optional client anchor (``person_id``/``household_id``); those enforce record
scope and can publish a guarded timeline event. Firm-level governance items (org/account/no anchor)
are visible to ``governance.view`` holders and record only to the append-only ``governance_events``
ledger + the shared ``audit_events`` hash-chain. Governance never mutates a canonical record.
"""
...
from app.db import engine, governance_events, people, record_assignments
...
# Approved governance lifecycle events published to the shared timeline (client-anchored only).
_TIMELINE_EVENTS = {"finding_opened": "governance_finding_opened",
                    "merge_approved": "governance_merge_approved",
                    "legal_hold_placed": "governance_legal_hold_placed",
                    "deletion_approved": "governance_deletion_approved",
                    "remediation_completed": "governance_remediation_completed"}
...
# --- scope (optional client anchor; firm-level items visible to governance.view) --------------
...
def record_event(c, *, entity_type, entity_id, event_type, from_status=None, to_status=None,
    ...
    c.execute(governance_events.insert().values(
...
def write_audit(action, *, entity_type, entity_id, actor_user_id=None, metadata=None):
    """Also record governance actions in the shared tamper-evident audit hash-chain."""
    try:
        import uuid
...
def publish_timeline(row: dict, kind: str, *, title=None, summary=None):
    """Publish an approved governance lifecycle event — only when the item carries a client anchor
    (the timeline requires person_id/household_id)."""
...
def audit_history(principal, *, entity_type, entity_id) -> list[dict]:
    with engine.connect() as c:
        return [dict(e) for e in c.execute(
            select(governance_events).where(governance_events.c.entity_type == entity_type,
                                            governance_events.c.entity_id == entity_id)
            .order_by(governance_events.c.occurred_at, governance_events.c.id)).mappings()]
