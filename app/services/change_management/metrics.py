"""Enterprise Change Management analytics readers (Phase D.63).

These are LOW-CARDINALITY operational counters ABOUT the change layer itself (how many dashboards / panels /
summaries were composed, failures) — registered into the SINGLE, existing Analytics Registry. This layer
defines NO change / release / deployment / configuration business metrics (those stay owned by their
authoritative owners) and NO second metrics registry. Never credentials, secrets, tokens, environment
variables, connection strings, private keys, deployment payloads, or sensitive configuration values.
"""
from __future__ import annotations

from . import registry, stats


def change_management_metrics(principal=None) -> dict:
    s = stats.change_stats()
    cov = registry.coverage()
    return {
        "dashboards_composed": s.get("dashboards_composed", 0),
        "panels_composed": s.get("panels_composed", 0),
        "summaries_composed": s.get("summaries_composed", 0),
        "aggregation_failures": s.get("aggregation_failures", 0),
        "authorization_failures": s.get("authorization_failures", 0),
        "restricted_panels": s.get("restricted_panels", 0),
        "avg_compose_ms": s.get("avg_compose_ms", 0.0),
        "by_dashboard": s.get("by_dashboard", {}),
        "change_domains": cov["change_domains"],
        "release_entries": cov["release_entries"],
        "configuration_entries": cov["configuration_entries"],
        "change_evidence": cov["change_evidence"],
        "dashboards": cov["dashboards"],
        "panels": cov["panels"],
    }


# --- readers for the platform Analytics registry (in-process counters; no DB, no PII) ---

def change_dashboards_composed(principal) -> int:
    return int(stats.change_stats().get("dashboards_composed", 0))


def change_panels_composed(principal) -> int:
    return int(stats.change_stats().get("panels_composed", 0))


def change_panel_failures(principal) -> int:
    return int(stats.change_stats().get("aggregation_failures", 0))


def change_authorization_failures(principal) -> int:
    return int(stats.change_stats().get("authorization_failures", 0))
