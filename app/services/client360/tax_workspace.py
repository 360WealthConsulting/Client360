...
def _drake_returns(conn, person_ids):
    ...
    # Deployment-order tolerance: the Drake schema is provisioned separately and may be absent in some
    # environments. Degrade to no Drake returns rather than raise (the call site is _safe-wrapped too).
    if conn.execute(text("SELECT to_regclass('public.drake_client_returns')")).scalar() is None:
        return []
