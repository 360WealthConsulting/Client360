"""Enterprise Capacity Planning read-models (Phase D.61).

Normalized, read-only projections of a composed workforce / capacity / utilization panel and a resource
dashboard. Every panel REFERENCES its authoritative owner (owning service + read + deep link) and never
assigns work, approves PTO, creates meetings, moves calendar events, schedules resources, or modifies
assignments — the value is composed on READ by the authoritative owner (Practice Management + the Operations
capacity owner, Work Management / the work queue, and Automation Orchestration). Every panel is explainable
...
panel is emitted ``available=False`` with ``config_status='not_configured'`` (fail closed). **No employee
details, payroll, HR records, calendar contents, time entries, or sensitive staffing data are ever carried in
a panel — counts, status, and coverage summaries only.** A derived value carries ``derived=True``, describes an
operational capacity/utilization summary (never a certified staffing figure), and never fabricates staffing,
utilization, availability, scheduling, or capacity information.
"""
...
class PanelResult:
    ...
    measure: str               # workforce | capacity | utilization | workload | staffing | queue | assignment | forecast
    ...
    derived: bool = False      # True when the value is a deterministic derivation (summary, never a certified figure)
