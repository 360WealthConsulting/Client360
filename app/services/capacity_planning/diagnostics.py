"""Enterprise Capacity Planning internal diagnostics (Phase D.61) — INTERNAL-ONLY observability of the
capacity layer. Composes the gate snapshot, in-process counters, registry coverage (incl. configured /
not_configured counts + staffing / utilization coverage), panel availability, and the governance report into
one low-cardinality report for the ``observability.audit`` surface. Exposes NO employee details, payroll, HR
records, calendar contents, time entries, or sensitive staffing data — aggregate metadata only.
"""
from __future__ import annotations

from . import gate, registry, stats
from .governance import validate_capacity_planning
...
def capacity_diagnostics() -> dict:
    gov = validate_capacity_planning()
    ...
    return {
        ...
        "staffing_coverage": cov["workforce_classes"],
        "utilization_coverage": cov["utilization_categories"],
