...
WHAT IT MAY CHANGE
-------------------
``documents.tax_year`` AND ``documents.tax_year_confidence``, on exactly the approved rows, where
both are currently NULL. The two travel together because migration cf01 enforces
``(tax_year IS NULL) = (tax_year_confidence IS NULL)`` — writing the year alone violates the schema,
so this is a two-column resolved-state write and is described as one everywhere.

Nothing else. Not tags, not ownership, not category/classification/subcategory, not folder_id, not
status or review_status, not display_name, not sha256, not OCR rows or ocr_status, not
document_sources, not document_folders. Every one of those is fingerprinted before and after and a
difference aborts, and both target fields are proved unchanged on every NON-target row.
...
#: The snapshot records the EXACT prior pair so a rollback restores what was there rather than
#: assuming NULL. The plan only admits NULL/NULL rows today, but a rollback script must not depend
#: on that remaining true — it restores values, not an assumption.
SNAPSHOT_COLUMNS = ["document_id", "previous_tax_year", "previous_tax_year_confidence",
                    "new_tax_year", "new_tax_year_confidence", "sha256", "owner_scope"]
...
#: Both target fields, proved unchanged on every row this batch does NOT own.
_NON_TARGET_TAX_YEAR_FP = ("select md5(string_agg(id::text || '|' || coalesce(tax_year::text,'') "
                           "|| '|' || coalesce(tax_year_confidence,''), '~' order by id)) "
                           "from documents where not (id = any(:ids))")
...
def run(candidate_csv, *, apply_changes=False, confirm=None, actor_user_id=None,
    ...
    try:
        ...
        locked = {r["id"]: dict(r) for r in connection.execute(text(
            "select id, tax_year, tax_year_confidence, sha256, status, archived, deleted_at, "
            "       person_id, household_id, organization_id from documents "
        ...
        for row in documents:
            ...
            if live["tax_year_confidence"] is not None:
                drift.append((document_id,
                              f"tax_year_confidence became {live['tax_year_confidence']!r}"))
        ...
        with snapshot.open("w", newline="", encoding="utf-8") as handle:
            ...
            writer.writeheader()
            for row in documents:
                live = locked[row["document_id"]]
                writer.writerow({
                    "document_id": row["document_id"],
                    "previous_tax_year": "" if live["tax_year"] is None else live["tax_year"],
                    "previous_tax_year_confidence": live["tax_year_confidence"] or "",
                    "new_tax_year": row["tax_year"],
                    "new_tax_year_confidence": row["tax_year_confidence"],
                    "sha256": row["sha256"], "owner_scope": row["owner_scope"]})
        ...
        for row in documents:
            # Both fields in one statement: cf01's pairing constraint is checked per row, so a
            # year-only write would fail on the first document. The WHERE clause re-asserts the
            # NULL/NULL invariant a third time, inside the write itself.
            updated = connection.execute(text(
                "update documents set tax_year = :year, tax_year_confidence = :confidence "
                " where id = :id and tax_year is null and tax_year_confidence is null "
                "returning id"),
                {"year": row["tax_year"], "confidence": row["tax_year_confidence"],
                 "id": row["document_id"]}).first()
            if updated is None:
            ...
            write_audit_event(conn=connection, action=AUDIT_ACTION,
                              ...
                              metadata={"batch_id": BATCH_ID, "plan_digest": digest,
                                        ...
                                        "tax_year_confidence": row["tax_year_confidence"],
                                        "previous_tax_year": locked[row["document_id"]]["tax_year"],
                                        "previous_tax_year_confidence":
                                            locked[row["document_id"]]["tax_year_confidence"],
        ...
        wrong = connection.execute(text(
            "select count(*) from documents d where d.id = any(:ids) "
            "  and (d.tax_year is null or d.tax_year_confidence is distinct from :confidence)"),
            {"ids": ids, "confidence": RESOLVED_CONFIDENCE}).scalar()
        if wrong:
            raise Abort(f"ABORT: {wrong} target documents do not carry the resolved pair "
                        f"(tax_year, tax_year_confidence='{RESOLVED_CONFIDENCE}')")
        ...
        _out(report, f"  COMMITTED {report['assigned']} resolved tax years "
                     f"(confidence '{RESOLVED_CONFIDENCE}'), {report['audit_rows']} audit rows")
        _out(report, "  post-write checks: every target carries the resolved pair, protected "
                     "fields unchanged, folder table unchanged, non-target tax_year and "
                     "tax_year_confidence unchanged")
