#!/usr/bin/env python3
"""Document filing persistence BATCH 4 — the guarded apply. ALL-OR-NOTHING.

    # default: reads, validates, reports, writes NOTHING
    python scripts/apply_document_filing_batch4.py --candidate <frozen csv>

    # writes, and only with every key turned at once
    python scripts/apply_document_filing_batch4.py --candidate <frozen csv> \\
        --actor-user-id 1 --confirm APPLY-DOCUMENT-FILING-BATCH4-1285 --apply

WHAT IT WRITES
--------------
The 357 folder rows this plan needs that do not exist yet, and ``documents.folder_id`` on exactly
1,285 documents. The other 15 nodes were created by Batch 4 and are REUSED: never renamed,
reparented, reclassified or recreated. No other document column is touched.

THE LANE
--------
Every row's only recorded filing problem was ``no_client_confirmation_in_path``. The preview could
not confirm the client from the path because it compares against ``first_name``/``last_name`` for a
person (NULL for all 80 people here, whose names live in ``full_name``) and against an
organization's full name including its legal suffix (which the path omits). Both are gaps in the
comparison, not evidence of an unconfirmed client, and
:mod:`app.services.document_filing_batch4` re-derives the corroboration deterministically instead of
assuming it. See that module for the exhaustive list of what is accepted and what is refused.

THE HARD PART IS NOT THE WRITE, IT IS THE TREE THAT IS ALREADY THERE
---------------------------------------------------------------------
Batch 1 could assert ``document_folders`` was empty. Batch 4 cannot: 3,150 rows are production
state, and a global count is far too weak a gate — it would pass while a folder had been renamed,
reparented or swapped underneath the plan. So this script proves the tree three ways instead:
...
* a fingerprint over every folder row EXCLUDING the ones being created must be byte-identical before
  and after, which catches a change to any pre-existing row, and the total must rise by exactly the
  number created.

A transaction-scoped advisory lock serialises two Batch 4 applies against each other, so the
"these folders are absent" finding cannot go stale between the check and the insert.

THE YEAR, BOTH WAYS ROUND
-------------------------
491 of these documents have a tax year the existing rules prove STRONGLY, and they file into a year
folder exactly as Batch 1's strong-year documents did. The other 794 file at the category, and no
year is chosen for them. The plan layer enforces the biconditional — a year node requires
``tax_year_confidence == 'strong'`` and a strong year requires the year node — so an uncertain year
can never be manufactured and a proven one can never be silently discarded. ``documents.tax_year``
is not written by this or any other batch; a year exists only as a folder.
"""
...
REPORT_ROOT = REPO_ROOT / "var" / "document_filing_batch4"

SNAPSHOT_CSV = "rollback_snapshot_document_filing_batch4.csv"
...
#: Batch 4 takes its actor from the CLI rather than pinning one, but an apply must still name a
...
#: Key for the transaction-scoped advisory lock that serialises Batch 4 applies.
ADVISORY_LOCK_KEY = 0x0DF12B04
...
def sha256_of(path: Path) -> str:
    from app.services.document_filing_batch4 import sha256_of as _sha
    return _sha(path)
    ...
    with path.open("w", newline="", encoding="utf-8") as fh:
        ...
        for document in sorted(plan["documents"], key=lambda d: d["document_id"]):
            ...
            writer.writerow({
                ...
                # delete it. A reused Batch 1 folder must survive a Batch 4 rollback.
...
def run(candidate_csv, *, apply_changes=False, confirm=None, actor_user_id=None,
    ...
    from app.services.document_filing_batch4 import (
        BATCH_NAME,
    ...
    census = plan["census"]
    out(f"  plan: {len(documents)} documents, {len(folders)} folder nodes "
        f"({census['client_nodes']} client, {census['category_nodes']} category, "
        f"{census['year_nodes']} year)")
    out("  destinations: " + ", ".join(f"depth {d}: {n}"
                                       for d, n in sorted(census["by_depth"].items())))
    out(f"  plan digest:   {plan['plan_digest']}")
    out(f"  folder digest: {plan['folder_manifest_digest']}")

    stamp = datetime.now(UTC).strftime("%Y%m%d-%H%M%S")
    out_dir = Path(output_root) / f"document-filing-batch4-{stamp}"
    ...
    try:
        ...
        # Serialise Batch 4 applies against each other for the life of this transaction, so the
        ...
        for document in documents:
            ...
            write_audit_event(
                ...
                metadata={
                    ...
                    # year would be stored as "[REDACTED]" and a NULL one read as a year that was
                    # recorded and hidden. The depth records the same fact in a form the audit
                    # trail keeps: 3 means this document went into a year folder, 2 means the
                    # batch declined to choose a year for it.
                    "destination_depth": document["depth"],
                },
                conn=connection)
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Apply the frozen document filing batch 4.")
    ap.add_argument("--candidate", required=True, help="the frozen batch 4 candidate CSV")
    ap.add_argument("--candidate-json", default=None,
                    help="the frozen batch 4 candidate JSON; its SHA is pinned when given")
