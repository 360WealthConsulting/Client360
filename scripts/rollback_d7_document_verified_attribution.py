#!/usr/bin/env python3
"""D7 document-corroborated attribution — the guarded rollback. ALL-OR-NOTHING.

    # default: reads, validates, reports, writes NOTHING
    python scripts/rollback_d7_document_verified_attribution.py --rollback-manifest <json>

    # writes, and only with every key turned at once
    python scripts/rollback_d7_document_verified_attribution.py --rollback-manifest <json> \\
        --confirm ROLLBACK-D7-DOCVERIFIED-<rows> --apply
...
erasure of the first: one compensating event is written per reversed row, through the same audit
writer the attribution service uses. That is the established contract here.

WHEN IT REFUSES
---------------
If a row was attributed to a different entity since the apply, if a person confirmed it, or if a
recorded link no longer looks like the row the apply created, the whole batch aborts. A rollback
that would discard someone else's later decision is worse than no rollback.
"""
...
#: Distinct from the apply's key and from Batch 1's, so the three cannot be confused by lock alone.
ADVISORY_LOCK_KEY = 0x0D7A0004
...
def confirm_phrase(rows: int) -> str:
    return f"ROLLBACK-D7-DOCVERIFIED-{int(rows)}"
    ...
    try:
        ...
        for row in rows:
            ...
            write_audit_event(
                ...
                request_id=f"rollback-d7-docverified-{dbi_id}", conn=connection,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="D7 document-corroborated attribution guarded rollback")
