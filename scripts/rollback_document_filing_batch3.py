#!/usr/bin/env python3
"""Document filing persistence BATCH 3 — the scoped rollback. ALL-OR-NOTHING.

    python scripts/rollback_document_filing_batch3.py --receipt <apply_receipt.json>
    python scripts/rollback_document_filing_batch3.py --receipt <apply_receipt.json> --apply \\
        --actor-user-id 1 --confirm ROLLBACK-DOCUMENT-FILING-BATCH3-12

THE DIFFERENCE FROM BATCH 1's ROLLBACK
---------------------------------------
Batch 1 created every folder it used, so reversing it meant deleting all of them. Batch 3 files
into a tree that earlier batches already populated, and whether a given node was created here or
reused is a fact about the run, not about the batch: this plan happens to create all 12 of its
nodes, but a rerun after a partial apply, or a later batch reaching the same client, would not.
So this rollback never infers what to delete. It deletes strictly the codes the RECEIPT records
as created, and the snapshot carries a per-row ``created_folder`` flag so a reused folder can
never be mistaken for one of ours. Deleting an earlier batch's folder would unfile that batch's
documents through ``ON DELETE SET NULL`` — silently, and with no record of where they had been.

WHAT IT RESTORES
----------------
Each of the 12 documents to the ``previous_folder_id`` the snapshot recorded, then the folders the
receipt records as created, children before parents. Never by relying on ``ON DELETE SET NULL``: that would destroy the
...
def run(receipt_path, *, candidate_csv=None, apply_changes=False, confirm=None, actor_user_id=None,
    ...
    from app.services.document_filing_batch3 import BATCH_NAME, rollback_phrase
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Reverse the document filing batch 3 apply.")
