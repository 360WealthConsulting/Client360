#!/usr/bin/env python3
"""Document filing persistence BATCH 2 — the guarded apply. ALL-OR-NOTHING.

    # default: reads, validates, reports, writes NOTHING
    python scripts/apply_document_filing_batch2.py --candidate <frozen csv>

    # writes, and only with every key turned at once
    python scripts/apply_document_filing_batch2.py --candidate <frozen csv> \\
        --actor-user-id 1 --confirm APPLY-DOCUMENT-FILING-BATCH2-550 --apply

WHAT IT WRITES
--------------
The FOUR folder rows this plan needs that do not exist yet, and ``documents.folder_id`` on exactly
550 documents. Every other node the plan needs — 377 of 381 — already exists because Batch 1 created
it, and this batch REUSES those rows: it never renames, reparents, reclassifies or recreates one.

THE HARD PART IS NOT THE WRITE, IT IS THE TREE THAT IS ALREADY THERE
---------------------------------------------------------------------
Batch 1 could assert ``document_folders`` was empty. Batch 2 cannot: 3,134 rows are production state,
...
* a fingerprint over every folder row EXCLUDING the four being created must be byte-identical before
  and after, which catches a change to any of the 3,134 existing rows, and the total must rise by
  exactly four.

A transaction-scoped advisory lock serialises two Batch 2 applies against each other, so the "absent"
finding cannot go stale between the check and the insert.

NO YEAR, ON PURPOSE
-------------------
These 550 documents were held back because their tax-year evidence contradicts itself. The plan layer
...
REPORT_ROOT = REPO_ROOT / "var" / "document_filing_batch2"

SNAPSHOT_CSV = "rollback_snapshot_document_filing_batch2.csv"
...
#: Key for the transaction-scoped advisory lock that serialises Batch 2 applies.
...
def sha256_of(path: Path) -> str:
    from app.services.document_filing_batch2 import sha256_of as _sha
    return _sha(path)


def confirm_phrase_for(plan) -> str:
    from app.services.document_filing_batch2 import confirm_phrase
    ...
    with path.open("w", newline="", encoding="utf-8") as fh:
        ...
        for document in sorted(plan["documents"], key=lambda d: d["document_id"]):
            ...
            writer.writerow({
                ...
                # delete it. A reused Batch 1 folder must survive a Batch 2 rollback.
...
def run(candidate_csv, *, apply_changes=False, confirm=None, actor_user_id=None,
    ...
    out(f"  sha256 verified: {plan['candidate_csv_sha256']}")
    ...
    out_dir = Path(output_root) / f"document-filing-batch2-{stamp}"
    ...
    try:
        ...
        # Serialise Batch 2 applies against each other for the life of this transaction, so the
        # "these four folders are absent" finding cannot go stale before the inserts.
        ...
        if any("--year-" in code for code in placed.values()):
            raise RuntimeError("a batch 2 document was filed into a YEAR folder")
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Apply the frozen document filing batch 2.")
    ap.add_argument("--candidate", required=True, help="the frozen batch 2 candidate CSV")
