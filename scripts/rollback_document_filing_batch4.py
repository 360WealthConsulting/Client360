#!/usr/bin/env python3
"""Document filing persistence BATCH 4 — the scoped rollback. ALL-OR-NOTHING.

    python scripts/rollback_document_filing_batch4.py --receipt <apply_receipt.json>
    python scripts/rollback_document_filing_batch4.py --receipt <apply_receipt.json> --apply \\
        --actor-user-id 1 --confirm ROLLBACK-DOCUMENT-FILING-BATCH4-1285

WHAT IT DELETES, AND WHAT IT REFUSES TO
----------------------------------------
Batch 1 created every folder it used, so reversing it meant deleting all of them. Batch 4 files into
a tree earlier batches already populated: 15 of its 372 nodes belong to Batch 4 and hold other
documents. Whether a given node was created here or reused is a fact about the RUN, not about the
batch, so this rollback never infers it. It deletes strictly the codes the RECEIPT records as
created, and the snapshot carries a per-row ``created_folder`` flag so a reused folder can never be
mistaken for one of ours. Deleting an earlier batch's folder would unfile that batch's documents
through ``ON DELETE SET NULL`` — silently, and with no record of where they had been.

WHAT IT RESTORES
----------------
Each of the 1,285 documents to the ``previous_folder_id`` the snapshot recorded, then the folders
the receipt records as created, children before parents — year nodes before their categories,
categories before their clients. Never by relying on ``ON DELETE SET NULL``: that would destroy the
...
def run(receipt_path, *, candidate_csv=None, apply_changes=False, confirm=None, actor_user_id=None,
    ...
    from app.services.document_filing_batch4 import BATCH_NAME, rollback_phrase
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Reverse the document filing batch 4 apply.")
