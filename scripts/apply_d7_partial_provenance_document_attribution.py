#!/usr/bin/env python3
"""D7 partial-provenance, document-corroborated attribution — the guarded apply. ALL-OR-NOTHING.

    # default: reads, validates, reports, writes NOTHING
    python scripts/apply_d7_partial_provenance_document_attribution.py

    # writes, and only with every key turned at once
    python scripts/apply_d7_partial_provenance_document_attribution.py \\
        --confirm APPLY-D7-PARTIALDOC-29 --apply

HOW THIS DIFFERS FROM THE OTHER TWO RUNNERS, AND WHY
----------------------------------------------------
``apply_d7_attribution_batch1`` requires at least two Drake source contacts.
``apply_d7_document_verified_attribution`` allows one contact but requires the entity's stored
provenance to cover the identifier's COMPLETE contact set.

These 29 rows satisfy neither. Their entities' stored provenance names only some of the contacts the
identifier appears on — typically the first filing year, with later years never added. The Drake
provenance gap is real and this runner does not pretend otherwise.

What closes the gap is evidence the provenance model does not hold: a document already filed under
the proposed entity, carrying a taxpayer identifier that derives to the same hash with the entity's
own name printed beside it, and in 27 of the 29 cases dated to the very year the provenance is
missing.

THE GAP IS NOT PAPERED OVER
---------------------------
A tempting shortcut would be to write the missing contact ids into
``relationship_entities.details`` and let the existing runner see complete coverage. This runner
does NOT do that, and refuses to: ``relationship_entities`` is in its forbidden list and is
fingerprinted before and after. Backfilling provenance would manufacture the evidence rather than
record it, and would leave the stored provenance claiming a corroboration Drake never supplied.

The gap stays visible in the data. The document evidence is recorded alongside it, in the audit
metadata and in the rollback receipt, as what it is: supplemental authorization.

WHAT COUNTS AS CORROBORATION
----------------------------
Identical to the deployed document-verified runner, and imported from it rather than restated, so
the two cannot drift apart: an identifier token not embedded in a longer numeric run, sitting within
60 normalised characters of the entity's own name. An identifier inside that window that does NOT
derive to the frozen hash is a contradiction and aborts the entire batch — the three rows that
showed contradictions during review are held out of this manifest and a test pins their exclusion.

THE RAW IDENTIFIER
------------------
Never printed, logged, or written to the manifest, the receipt or audit metadata.
"""
...
from scripts.apply_d7_document_verified_attribution import (  # noqa: E402
    IDENTIFIER_TOKEN,
    NAME_ADJACENCY,
    extract_pdf_text,
    flat,
...
ARTIFACT_DIR = REPO_ROOT / "reports" / "d7_partial_doc"
DEFAULT_MANIFEST = ARTIFACT_DIR / "d7_partial_doc_batch_manifest.csv"
DEFAULT_SIDECAR = ARTIFACT_DIR / "d7_partial_doc_batch_manifest.json"

#: Filled in by the freeze. Both files are checked; neither alone catches every tampering.
APPROVED_MANIFEST_SHA256 = "6df4bc90a1ffc5ee8ea7e2a14d5bfa40d86a158895a855806f585e25b261d08a"
APPROVED_SIDECAR_SHA256 = "1ef7367cd41f305c8215abf1ec4fd9974ac334ae3ad1ce6e68601854b8255ffb"
APPROVED_PLAN_DIGEST = "162155d553791a548410842b0e46df6071cece0a91b5f4e1f54345a8e1739feb"
APPROVED_ROWS = 29

#: Its own key. Three D7 applies now exist and none may be mistaken for another by lock alone.
ADVISORY_LOCK_KEY = 0x0D7A0005

#: relationship_entities is here deliberately: this runner must never close the provenance gap by
#: writing to it. See the module docstring.
...
MANIFEST_COLUMNS = ("dbi_id", "identifier_hash", "subject_name", "subject_type",
                    ...
                    "source_contact_count", "missing_source_contact_ids", "coverage_numerator",
                    "coverage_denominator", "complete_coverage", "document_evidence_count",
                    "contradiction_count", "expected_esl_inserts", "expected_trust_level",
                    "expected_confirmation_source", "expected_evidence_method",
                    "dbi_row_sha256", "target_entity_sha256")

#: Every row here is partial by construction; a complete-coverage row belongs to the other runner.
REQUIRE_PARTIAL_COVERAGE = True
DOCUMENT_CORROBORATION_REQUIRED = True
MAX_CONTRADICTIONS = 0

#: These stay out. Three of them showed contradictory document evidence during review.
HELD_DBI_IDS = frozenset({531, 533, 552, 555, 563, 578, 589, 620, 637})
CONTRADICTION_DBI_IDS = frozenset({555, 578, 620})
...
def confirm_phrase(rows: int) -> str:
    return f"APPLY-D7-PARTIALDOC-{int(rows)}"
...
def plan_digest(rows, sidecar_by_id) -> str:
    """The reviewed plan, including the documents that authorize each row."""
    return hashlib.sha256(canon([
        {"dbi_id": int(r["dbi_id"]), "identifier_hash": r["identifier_hash"],
         ...
         "contacts": _ints(r["source_contact_ids"]),
         "documents": sorted(ev["document_id"] for ev in
                             sidecar_by_id[int(r["dbi_id"])]["document_evidence"])}
...
def load_manifest(manifest_path=DEFAULT_MANIFEST, sidecar_path=DEFAULT_SIDECAR, *,
    ...
    """Read and gate the frozen pair. No connection is opened until this returns."""
    ...
    side = sha256_of(sidecar_path)
    if expect_sidecar and side != expect_sidecar:
        raise Abort(f"ABORT: sidecar SHA256 {side} != approved {expect_sidecar}")
    ...
    held = sorted(set(ids) & HELD_DBI_IDS)
    if held:
        raise Abort(f"ABORT: manifest contains held row(s) {held}; these are excluded from this "
                    "batch and three of them carry contradictory document evidence")
    ...
    for row in rows:
        ...
        if REQUIRE_PARTIAL_COVERAGE:
            if str(row["complete_coverage"]).lower() != "false":
                raise Abort(f"ABORT: dbi {dbi_id} claims complete coverage; a complete-coverage "
                            "row belongs to the document-verified runner, not this one")
            if not _ints(row["missing_source_contact_ids"]):
                raise Abort(f"ABORT: dbi {dbi_id} names no missing contact, so it is not partial")
        if int(row["contradiction_count"]) > MAX_CONTRADICTIONS:
            raise Abort(f"ABORT: dbi {dbi_id} carries {row['contradiction_count']} recorded "
                        "contradiction(s)")
        evidence = by_id[dbi_id].get("document_evidence") or []
        if DOCUMENT_CORROBORATION_REQUIRED and not evidence:
            raise Abort(f"ABORT: dbi {dbi_id} has no document evidence; a partial-coverage row is "
                        "admissible here only with it")
        if int(row["document_evidence_count"]) != len(evidence):
...
def check_documents(connection, text, derive, row, sidecar_row, *, extract=extract_pdf_text) -> list:
    """Every authorizing document must still say what it said. One that does not aborts the batch."""
    dbi_id = int(row["dbi_id"])
    ...
    confirmed = []
    for doc in sidecar_row.get("document_evidence") or []:
        ...
        body = extract(path)
        if body is None:
            raise Abort(f"ABORT: dbi {dbi_id} document {doc_id} could not be read; unreadable "
                        "evidence is refused, never treated as absent")
        body = flat(body)
        anchors = [m.start() for m in re.finditer(re.escape(flat(entity_name)), body)]
        if not anchors:
            raise Abort(f"ABORT: dbi {dbi_id} document {doc_id} no longer names entity "
                        f"{entity_id}; a name-free document cannot corroborate it")
        ...
        confirmed.append({"document_id": doc_id, "document_path": path,
                          "document_sha256": actual, "document_year": doc.get("document_year", ""),
                          "corroborating_occurrences": corroborating})
...
def _revalidate(connection, text, row) -> None:
    ...
    entity = connection.execute(text(
        "select id, active, details from relationship_entities where id = :e"),
    ...
    # The provenance gap is expected. What must not change is WHICH contacts it covers, because the
    # frozen row records the gap the document evidence was reviewed against.
    details = entity["details"]
    if isinstance(details, str):
        details = json.loads(details or "{}")
    covered = {int(x) for x in ((details or {}).get("source_contact_ids") or [])}
    if sorted(covered & set(contacts)) != sorted(_ints(row["provenance_covered_contacts"])):
        raise Abort(f"ABORT: dbi {dbi_id} entity provenance coverage changed since the freeze")
    if sorted(set(contacts) - covered) != sorted(_ints(row["missing_source_contact_ids"])):
        raise Abort(f"ABORT: dbi {dbi_id} missing-contact set changed since the freeze")
...
def run(manifest_path=DEFAULT_MANIFEST, sidecar_path=DEFAULT_SIDECAR, *, apply_changes=False,
        ...
        expect_digest=APPROVED_PLAN_DIGEST, expect_rows=APPROVED_ROWS, output_root=REPORT_ROOT,
    ...
    by_id = {int(r["dbi_id"]): r for r in sidecar["rows"]}
    digest = plan_digest(rows, by_id)
    want = confirm_phrase(len(rows))
    expected_links = sum(len(_ints(r["source_contact_ids"])) for r in rows)
    expected_docs = sum(len(by_id[int(r["dbi_id"])]["document_evidence"]) for r in rows)

    out(f"frozen manifest: {manifest_path}")
    out(f"  sha256   : {sha256_of(manifest_path)}")
    out(f"  sidecar  : {sha256_of(sidecar_path)}")
    out(f"  rows     : {len(rows)}  contacts: {expected_links}  documents: {expected_docs}")
    out(f"  digest   : {digest}")
    ...
    out_dir = Path(output_root) / f"d7-partialdoc-{stamp}"
    report = {"batch": BATCH_ID, "rows": len(rows), "manifest": str(manifest_path),
              "manifest_sha256": sha256_of(manifest_path),
              "sidecar_sha256": sha256_of(sidecar_path), "plan_digest": digest,
              "attributed": 0, "links_created": 0, "documents_verified": 0,
              "committed": False, "refusals": [], "report_dir": str(out_dir),
              "rollback_manifest": None, "rollback_manifest_sha256": None}

    # REPEATABLE READ so the before/after forbidden-table fingerprints compare like with like.
    ...
    try:
        ...
        for row in rows:
            ...
            evidence[int(row["dbi_id"])] = check_documents(
                connection, text, derive, row, by_id[int(row["dbi_id"])], extract=extract)
        report["documents_verified"] = sum(len(v) for v in evidence.values())
        out(f"  revalidated {len(rows)}/{len(rows)} rows; "
            f"{report['documents_verified']} authorizing document(s) re-verified")
        ...
        requests = [(row, AttributionRequest(
            ...
            reason=f"{BATCH_ID} (plan {digest[:12]}; "
                   f"{len(evidence[int(row['dbi_id'])])} corroborating document(s))"))
            for row in rows]
        ...
        rollback_path = out_dir / "partialdoc_rollback_manifest.json"
...
def _check_forbidden(connection, text, before_fp) -> None:
    ...
    if changed:
        detail = "; ".join(f"{t}: {before_fp[t].split('/')[0]} -> {after_fp[t].split('/')[0]} rows"
                           for t in changed)
        raise Abort(f"ABORT: forbidden table(s) changed: {changed} ({detail}). This runner never "
                    "closes the provenance gap by writing to relationship_entities.")
...
def _rollback_payload(connection, text, rows, results, report, digest, evidence) -> dict:
    payload = {"batch": BATCH_ID, "created_at": datetime.now(UTC).isoformat(),
               ...
               "sidecar_sha256": report["sidecar_sha256"], "plan_digest": digest, "rows": []}
    for row, result in results:
        ...
        payload["rows"].append({
            ...
            "missing_source_contact_ids": _ints(row["missing_source_contact_ids"]),
    ...
    if IDENTIFIER_TOKEN.search(canon(payload)):
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="D7 partial-provenance document-corroborated attribution guarded apply")
